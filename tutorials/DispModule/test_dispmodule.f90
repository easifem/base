PROGRAM TEST_DISPMODULE

  ! MAIN TEST PROGRAM FOR DISPMODULE. SEE ALSO test_dispmodule_fpp.F90
  ...
  character(3)  :: adv = 'no'
  character(90) :: fmt = '("  Testing dispmodule, int kind=",I0,", real kind=",I0,", logical kind=",I0,"...")'
...
CONTAINS
  ...
  subroutine disp_xy(x,y)
    ...
    ! use dispmodule (already "used" here, but explicit in manual)
    real, intent(in) :: x(:,:), y(:,:)
    type(disp_settings) ds
    ds = disp_get()
