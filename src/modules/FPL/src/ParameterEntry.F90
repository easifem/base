...
contains
  ...
  procedure, public, non_overridable :: PointToValue => &
    & EntryListIterator_PointToValue
...
contains
  ...
  procedure, non_overridable, public :: NullifyNext => &
    & ParameterEntry_NullifyNext
  ...
  procedure, non_overridable, public :: DeallocateKey => &
    & ParameterEntry_DeallocateKey
  ...
  procedure, non_overridable, public :: DeallocateValue => &
    & ParameterEntry_DeallocateValue
  procedure, non_overridable, public :: PointToValue => &
    & ParameterEntry_PointToValue
  procedure, non_overridable, public :: GetIterator => &
    & ParameterEntry_GetIterator
...
subroutine ParameterEntry_Print(this, unit, prefix, iostat, iomsg)
  ...
  if (this%HasKey()) then
    ...
    write (unit=unit, fmt='(A)', advance="NO", iostat=iostatd, &
      & iomsg=iomsgd) prefd//' Key = "'//Key//'", '
    !!
    select type (Wrapper => this%Value)
    class is (DimensionsWrapper_t)
      call Wrapper%Print(unit=unit)
    class Default
      write (unit=unit, fmt='(A)', iostat=iostatd, iomsg=iomsgd) &
        & ' is a Parameter SubList'
    end select
  end if
