...
int my_access(const char *pathname, int which) {
  int n;
  /*fprintf(stdout," which values = %d %d %d %d
...
void my_readdir(DIR *dirp, char *filename, int *ierr) {
  ...
  *ierr = 0;
  ...
  if ((dp = readdir(dirp)) != NULL) {
    ...
    memcpy(filename, dp->d_name, length);
  } else {
    *ierr = -1; /*When the end of the directory is encountered, a null pointer
...
/*--------------------------------------------------------------------------------------------------------------------------------*/
char *my_realpath(char *symlinkpath) { return (realpath(symlinkpath, NULL)); }
/*--------------------------------------------------------------------------------------------------------------------------------*/
/*
...
void my_readenv(char *variable) {
  ...
  if (*ep == NULL) {
    ...
    */
    ...
    memcpy(variable, *ep, length_copy + 1);
    *ep++;
  }
}
/*--------------------------------------------------------------------------------------------------------------------------------*/
...
void system_unbuffer() {
  ...
  */
  char ident[] = "@(#)system_unbuffer(3c): FORTRAN-callable call to turn off "
                 "buffering of stdin";
...
void my_uname(char *which, char *string, int *stringlen) {
  ...
  if (uname(&name) == -1) {
    ...
    switch (*which) {
    case 's':
      strncpy(string, name.sysname, *stringlen);
      break;
    case 'n':
      strncpy(string, name.nodename, *stringlen);
      break;
    case 'r':
      strncpy(string, name.release, *stringlen);
      break;
    case 'v':
      strncpy(string, name.version, *stringlen);
      break;
    case 'm':
      strncpy(string, name.machine, *stringlen);
      break;
    case 'T':
...
#include <sys/times.h>
  ...
  *u = ((float)mytime.tms_utime) /
       ((float)HZ); /* user time in 1/HZ seconds is in tms_utime */
...
void my_stat(char *file, long int *values, int *ierr, int debug) {
  struct stat *buf =
      malloc(sizeof(struct stat)); /* allocates memory for stat structure. */
  struct passwd *pwd;
  struct group *grp;
  struct tm *tm;
  char datestring[256];
  ...
  *ierr = 0;
  ...
  values[6] = buf->st_rdev; /* st_rdev   ID of device containing directory entry
                               for file (0 if not available) */
  values[7] = buf->st_size; /* st_size   file size in bytes */
  values[8] =
      buf->st_atime + 0.5; /* st_atime  most recent time file was accessed. */
  values[9] = buf->st_mtime +
              0.5; /* st_mtime  most recent time file contents modified. */
  values[10] = buf->st_ctime +
               0.5; /* st_ctime  most recent time file permissions changed. */
  values[11] = buf->st_blksize; /*           Preferred I/O block size (-1 if not
                                   available) */
  values[12] = buf->st_blocks;  /*           Number of blocks allocated (-1 if
                                   not available) */
  ...
  if (debug == 0) {
    ...
    /*
      ...
      (*note Symbolic Modes::). But ls(1) combines multiple bits into the third
      character of each set of permissions as follows: s    If the set-user-ID
      or set-group-ID bit and the corresponding executable bit are both set. S
      If the set-user-ID or set-group-ID bit is set but the corresponding
      executable bit is not set. t    If the restricted deletion flag or sticky
      bit, and the other-executable bit, are both set. The restricted deletion
            flag is another name for the sticky bit. *Note Mode Structure::.
           T    If the restricted deletion flag or sticky bit is set but the
      other-executable bit is not set. x    If the executable bit is set and
      ...
      GNU ls(1) uses a . character to indicate a file with a security context,
    ...
    */
    if (S_ISDIR(buf->st_mode)) { /* st_mode      The current permissions on the
                                    file. */
      local_buff[i] = 'd';
    ...
    i++;
    local_buff[i] = (buf->st_mode & S_IRUSR) ? 'r' : '-';
    i++;
    local_buff[i] = (buf->st_mode & S_IWUSR) ? 'w' : '-';
    i++;
    local_buff[i] = (buf->st_mode & S_IXUSR) ? 'x' : '-';
    i++;
    local_buff[i] = (buf->st_mode & S_IRGRP) ? 'r' : '-';
    i++;
    local_buff[i] = (buf->st_mode & S_IWGRP) ? 'w' : '-';
    i++;
    local_buff[i] = (buf->st_mode & S_IXGRP) ? 'x' : '-';
    i++;
    local_buff[i] = (buf->st_mode & S_IROTH) ? 'r' : '-';
    i++;
    local_buff[i] = (buf->st_mode & S_IWOTH) ? 'w' : '-';
    i++;
    local_buff[i] = (buf->st_mode & S_IXOTH) ? 'x' : '-';
    i++;
    local_buff[i] = '\0';
    ...
    if ((pwd = getpwuid(buf->st_uid)) !=
        NULL) { /* st_uid          The User ID for the file. */
      printf("Owner ........................... %-8s\n", pwd->pw_name);
    }
    printf("Owner ID ........................ %-8d\n", buf->st_uid);
    /* Print out group name if found using getgrgid(). */
    if ((grp = getgrgid(buf->st_gid)) !=
        NULL) { /* st_gid          The Group ID for the file. */
      printf("Group name ...................... %-8s\n", grp->gr_name);
    }
    printf("Group ID ........................ %-8d\n", buf->st_gid);

    tm = localtime(&buf->st_mtime); /* st_mtime  most recent time file contents
                                       modified. */
    strftime(datestring, sizeof(datestring), nl_langinfo(D_T_FMT),
             tm); /* Get localized date string. */
    printf("file contents last modified ..... %s\n", datestring);

    tm = localtime(
        &buf->st_atime); /* st_atime  most recent time file was accessed. */
    strftime(datestring, sizeof(datestring), nl_langinfo(D_T_FMT),
             tm); /* Get localized date string. */
    printf("file contents last accessed ..... %s\n", datestring);

    tm = localtime(&buf->st_ctime); /* st_ctime  most recent time file
                                       permissions changed. */
    strftime(datestring, sizeof(datestring), nl_langinfo(D_T_FMT),
             tm); /* Get localized date string. */
...
/* ===============================================================================================================================*/
/*
   To get stream I/O out of stdin and stdout, make a getc and putc callable from
   Fortran
*/
char getkeyC(void) {
