...
! date: 	22 March 2021
! summary: 	This submodule contains method for setting values in [[SparseMatrix_]]
...
MODULE PROCEDURE setSparsity_1
  ...
  CASE DEFAULT
    ...
    ELSE
      ...
      DO b = 1, Obj%tDOF
        Obj%nnz = Obj%nnz + n
        CALL APPEND( Obj%Row( ( b - 1 ) * Obj%tNodes( b )+Row)%Val,n2ntemp )
      END DO
      !
      DEALLOCATE( n2ntemp )
    END IF
...
!> authors: Vikas Sharma, Ph. D.
! date: 	22 March 2021
! summary: This subroutine set the value in `sparsematrix_`
!
!### Introduction
...
PURE SUBROUTINE setValueInternally( Obj, Nptrs, Val )
  ...
  CASE DEFAULT
    DO j = 1, Obj%tDOf
      DO i = 1, n
        row( ( j - 1 ) * n + i ) = ( j - 1 ) * Obj%tNodes( j ) + nptrs( i )
      END DO
    END DO
  END SELECT
