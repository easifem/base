...
PURE SUBROUTINE Vector_Scalar_Master(obj1, obj2, ans, varCase)
...
PURE SUBROUTINE constant_space(obj1, obj2, ans)
  ...
  DO CONCURRENT(ii=1:nsd, jj=1:np)
    ans%val(FortranIndex(ii, jj, nsd, np)) = obj1%val(ii) _OP_ &
                                             obj2%val(jj)
  END DO
...
PURE SUBROUTINE constant_time(obj1, obj2, ans)
  ...
  DO CONCURRENT(ii=1:nsd, kk=1:nnt)
    ans%val(FortranIndex(ii, kk, nsd, nnt)) = obj1%val(ii) _OP_ &
                                              obj2%val(kk)
  END DO
