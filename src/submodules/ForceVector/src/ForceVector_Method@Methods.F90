...
DO ips = 1, test%nips
  realval = test%js(ips) * test%ws(ips) * test%thickness(ips) * scale0
...
DO ips = 1, test%nips
  realval = scale0 * test%js(ips) * test%ws(ips) * test%thickness(ips)
...
CALL ForceVector_( &
  N=N(a:b, :), js=js, ws=ws, thickness=thickness, nns=mynns, &
  nips=nips, c=c, ans=ans, tsize=tsize)
