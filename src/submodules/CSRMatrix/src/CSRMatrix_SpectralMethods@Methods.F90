...
DO
  ...
  IF (ido .EQ. -1 .OR. ido .EQ. 1) THEN
    ...
    IF (info1 .NE. 0) THEN
      CALL ErrorMsg( &
        & msg="Error occured in LinSolve() errorCode="//tostring(info1), &
        & file=__FILE__, &
        & line=__LINE__, &
...
INTEGER(I4B) :: ido, lworkl, ldv, info, iparam(11), &
  & ipntr(11), maxIter0, n, ncv0, ii
CHARACTER(1), PARAMETER :: bmat = "I"
CHARACTER(2) :: which0
REAL(DFP) :: tol0, sigma0
REAL(DFP), ALLOCATABLE :: resid(:), v(:, :), workd(:), workl(:)
TYPE(String) :: err_msg
LOGICAL(LGT), ALLOCATABLE :: SELECT(:)
INTEGER(I4B) :: info1
