...
END IF
  ...
  !! check
  !!
IF (jvar .GT. (.tNames.obj%csr%jdof)) THEN
  CALL ErrorMSG(  &
...
END IF
  ...
  !!
c = getNodeLoc(obj=obj%csr%jdof, idof=(obj%csr%jdof.DOFStartIndex.jvar))
col_start = c(1) ! start
c = getNodeLoc(obj%csr%jdof, (obj%csr%jdof.DOFEndIndex.jvar))
col_end = c(2) ! end
  !!
  !!
  !!
